...
// ============================================================================
// OPTIMIZATION #1: Smart GPU sync waiter (replaces dumb yield loop)
// ============================================================================
namespace {
    bool WaitForGpuEvent(ID3D11DeviceContext* context,
                         ID3D11Query* event_query,
                         int timeout_ms = 20)
    {
        if (!event_query) {
            context->Flush();
            return true;  // No query available, just flush
        }

        // Phase 1: Busy-wait for < 1ms (GPU usually done immediately)
        // Using _mm_pause() instead of yield() to reduce context switches
        for (int i = 0; i < 2000; ++i) {
            BOOL data = FALSE;
            if (context->GetData(event_query, &data, sizeof(BOOL), 0) == S_OK) {
                return true;  // Early exit - GPU work is done
            }
            _mm_pause();  // CPU pause hint (better than yield)
        }

        // Phase 2: Sleep-based wait with deadline
        ...
        return false;  // Timeout - GPU may be stuck
    }
}

// ============================================================================
// OPTIMIZATION #2: Parallel RGB→NV12 conversion (multi-threaded)
// ============================================================================
void ConvertPackedRgbToNV12_Parallel(
    ...
    std::vector<uint8_t>& output)
{
    ...
    // Don't parallelize for small textures (threading overhead > benefit)
    if (height < 256 || num_threads < 2) {
        // Fall back to single-threaded version from original code
    ...
    // Parallel path for large textures
    num_threads = std::min(num_threads, (height + 127) / 128);  // 1 thread per 128 rows

    std::vector<std::thread> threads;
    const uint32_t chunk_height = (height + num_threads - 1) / num_threads;

    for (unsigned int t = 0; t < num_threads; ++t) {
        threads.emplace_back([&](unsigned int tid) {
            uint32_t start_y = tid * chunk_height;
            uint32_t end_y = std::min(start_y + chunk_height, height);

            // NV12 requires even row processing
            if (start_y % 2 != 0) ++start_y;
            if (start_y >= end_y) return;

            for (uint32_t y = start_y; y < end_y; y += 2) {
                const uint8_t* row0 = source + (static_cast<size_t>(y) * source_stride);
                const uint8_t* row1 = source + 
                    (static_cast<size_t>(std::min(y + 1, height - 1)) * source_stride);

                for (uint32_t x = 0; x < width; x += 2) {
                    int u_accum = 0, v_accum = 0;
                    ...
                    for (int dy = 0; dy < 2; ++dy) {
                        ...
                        for (int dx = 0; dx < 2; ++dx) {
                            ...
                            int r = 0, g = 0, b = 0;
                            ReadRgbPixel(pixel, format, r, g, b);
                    ...
                    const size_t uv_idx = (static_cast<size_t>(y / 2) * width) + x;
                    uv_plane[uv_idx] = ClampToByte(u_accum / 4);
                    uv_plane[uv_idx + 1] = ClampToByte(v_accum / 4);
                }
            }
        }, t);
    }
...
void MediaFoundationEncoder::Shutdown() {
    ...
    m_nv12BufferPool[0].clear();
    m_nv12BufferPool[1].clear();
    m_lastError.clear();
    m_sequenceHeaderAnnexB.clear();
    m_avccLengthFieldBytes = 4;
    m_sentSequenceHeader = false;
...
void MediaFoundationEncoder::RegisterEncodeFailure(const char* stage) {
    ++m_encodeFailures;
    ++m_consecutiveEncodeFailures;
    if (stage && std::strcmp(stage, "process_input") == 0) {
        ++m_processInputFailures;
    } else if (stage && std::strcmp(stage, "process_output") == 0) {
        ++m_processOutputFailures;
    }

    if (m_consecutiveEncodeFailures >= kFailureCooldownThreshold) {
        m_cooldownUntil = std::chrono::steady_clock::now() +
            std::chrono::milliseconds(kFailureCooldownMs);
        if (m_logger) {
            m_logger("OpenDriver [DX11]: Consecutive encode failures detected, applying short cooldown");
        }
    }
}

void MediaFoundationEncoder::RegisterEncodeSuccess() {
    m_consecutiveEncodeFailures = 0;
}

bool MediaFoundationEncoder::IsInFailureCooldown() const {
    return std::chrono::steady_clock::now() < m_cooldownUntil;
}

void MediaFoundationEncoder::MaybeLogTelemetry(double encode_ms,
                                               size_t packet_size_bytes,
                                               bool produced_output) {
    if (!m_logger) {
        return;
    }
    ++m_telemetryLogCounter;
    if ((m_telemetryLogCounter % kTelemetryLogInterval) != 0) {
        return;
    }
    ...
    std::snprintf(
        ...
        static_cast<unsigned long long>(m_outputSamplesProduced),
...
// ============================================================================
// OPTIMIZATION #3: Buffer pool — alternate between 2 buffers to reduce reallocations
// ============================================================================
std::vector<uint8_t>& MediaFoundationEncoder::GetPooledNV12Buffer(size_t required_size) {
    ...
    bool MediaFoundationEncoder::EncodeFrame(void* texture_handle, std::vector<uint8_t>& out_packet) {
        ...
        if (IsInFailureCooldown()) {
            ++m_cooldownSkips;
            SetLastError("Encoder cooldown active after repeated failures");
            return false;
        }
        ...
        if (can_try_zero_copy) {
            sample_ready = CreateSampleFromTexture(shared_texture.Get(), sample.GetAddressOf());
            if (!sample_ready) {
                ++m_zeroCopyFailures;
                if (m_zeroCopyFailures >= kZeroCopyFailureThreshold) {
                    m_allowZeroCopyInput = false;
                    if (m_logger) {
                        m_logger("OpenDriver [DX11]: Zero-copy input unstable, switching to NV12 fallback path");
                    }
                }
            }
        }
        ...
        if (!sample_ready) {
            if (!ConvertTextureToNV12Sample(shared_texture.Get(), desc.Format, sample.GetAddressOf())) {
                return false;
            }
            sample_ready = true;
        }
        ...
        if (FAILED(hr)) {
            if (can_try_zero_copy) {
                ...
                if (m_zeroCopyFailures >= kZeroCopyFailureThreshold) {
                    m_allowZeroCopyInput = false;
                    if (m_logger) {
                        m_logger("OpenDriver [DX11]: ProcessInput failed repeatedly on zero-copy path, forcing NV12 fallback");
                    }
                }
            }
            RegisterEncodeFailure("process_input");
            SetLastError("ProcessInput failed " + HrToHex(hr));
            return false;
        }
        if (can_try_zero_copy && m_zeroCopyFailures > 0) {
            m_zeroCopyFailures = 0;
        }
        ...
        if (produced) {
            ...
            RegisterEncodeFailure("process_output");
        }
        MaybeLogTelemetry(encode_ms, out_packet.size(), produced);
        return produced;
    }
...
bool MediaFoundationEncoder::SetupEncoderMFT(const VideoConfig& config) {
    ...
    for (const SearchPass& pass : passes) {
        ...
        for (UINT32 i = 0; i < activate_count; ++i) {
            ...
            // OPTIMIZATION: Adaptive GOP size based on resolution
            ...
            if (SUCCEEDED(candidate.As(&codec_api))) {
                ...
                // Multi-slice for better parallelism
                if (area >= 1920 * 1080) {
#ifdef CODECAPI_AVEncVideoNumSlices
                    var.ulVal = std::min(8u, (config.width + 255) / 256);
            ...
            for (const InputSubtype& input_subtype : input_subtypes) {
                if (input_type_name == input_subtype.name &&
                    input_subtype.guid == MFVideoFormat_NV12) {
                    m_requiresNv12Input = true;
                    break;
                }
            }
            m_sequenceHeaderAnnexB.clear();
...
bool MediaFoundationEncoder::ConvertTextureToNV12Sample(ID3D11Texture2D* sourceTexture,
    ...
    // OPTIMIZATION: Smart GPU sync wait (replaces dumb yield loop)
    if (m_query) {
        ...
        if (!WaitForGpuEvent(m_d3dContext.Get(), m_query.Get(), 20)) {
            if (m_logger) {
                m_logger("OpenDriver [DX11]: GPU sync timeout - frame may be stale");
            }
        }
    } else {
        m_d3dContext->Flush();
    ...
    // We use hardware MFT where available, but feed it from system-memory NV12.

    // Software MFT / fallback path
    Microsoft::WRL::ComPtr<IMFMediaBuffer> buffer;
    hr = MFCreateMemoryBuffer(static_cast<DWORD>(nv12_buf.size()), &buffer);
    if (FAILED(hr)) {
    ...
    std::memcpy(destination, nv12_buf.data(), nv12_buf.size());
    buffer->Unlock();

    hr = buffer->SetCurrentLength(static_cast<DWORD>(nv12_buf.size()));
    if (FAILED(hr)) {
